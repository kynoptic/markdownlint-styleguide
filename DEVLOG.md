...
- Calibrated against every distinct commit subject reachable from branches, remotes and tags — `git log --format=%s --branches --remotes --tags | sort -u`, 626 of them when the check was written and 630 now that more have landed: exactly 5 fail the new check. The ref scope is part of the number, so re-derive it with that command rather than `git log --all`, which also reads `refs/stash` and reports 632 distinct and 7 failures; the two extras are stash subjects (`On main: …`, `index on main: …`), and a stash commit never reaches a `commit-msg` hook. Three are version-only subjects from before the convention existed (`0.2.0`, `0.2.1`, `0.2.2`) and two use types the convention does not define, `config:` and `cleanup:`, both of which the convention would have written as `chore:`.
