...
- Worth knowing what this does not cover: nothing enforces the `<type>:` prefix itself. `check-commit-body.sh` never checks it. The claim that it does is in `.husky/commit-msg`, whose comment reads "Enforce Conventional Commits format and body style"; the script's own header says "Enforce commit message conventions" and then lists accurately what it enforces, so the misattribution was mine, not the script's. Deleting an unenforced checkbox does not change that; adding the check to the hook would.
